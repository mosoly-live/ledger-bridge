...
// No emojis, numbers or symbols.
type OnlyLetters string

// Validate validates string to be only letters and nothing else.
func (ol OnlyLetters) Validate(formats strfmt.Registry) error {
	m := string(ol)
	// Ensure that string is of eligible length.
	if err := validate.MinLength("", "body", m, 1); err != nil {
		return err
	}
	if err := validate.MaxLength("", "body", m, 128); err != nil {
		return err
	}
	// Ensure that expression has valid spacing and contains no numbers
	// or frequently used symbols.
	r, err := regexp.Compile(acceptedName)
	if err != nil {
		return err
	}
	if !r.MatchString(m) {
