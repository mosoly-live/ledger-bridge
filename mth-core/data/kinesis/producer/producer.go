// Package producer is a KPL-like batch producer for Amazon Kinesis built on top of the official Go AWS SDK
// and using the same aggregation format that KPL use.
//
// Note: This project started as a fork of `tj/go-kinesis`. If you are not interested in the
// KPL aggregation logic, you will probably want to check it out.
package producer
	...
	if nbytes > p.AggregateBatchSize {
		...
		if needToDrain {
			if record, err = p.aggregator.Drain(); err != nil {
				p.Logger.With(zap.Error(err)).Error("drain aggregator")
			}
		}
...
func (p *Producer) Start() {
	p.Logger.With(zap.String("stream", p.StreamName)).Info("starting producer")
	go p.loop()
}
...
func (p *Producer) Stop() {
	...
	p.stopped = true
	p.Unlock()
	p.Logger.With(zap.Int("backlog", len(p.records))).Info("stopping producer")
...
func (p *Producer) flush(records []*kinesis.PutRecordsRequestEntry, reason string) {
	...
	for {
		p.Logger.With(zap.String("reason", reason)).Info(fmt.Sprintf("flush %v records", len(records)))
		...
		if err != nil {
			p.Logger.With(zap.Error(err)).Error("flush")
			p.RLock()
			notify := p.notify
			p.RUnlock()
			if notify {
		...
		if p.Verbose {
			for i, r := range out.Records {
				...
				if r.ErrorCode != nil {
					l = l.With(
						zap.String("ErrorCode", *r.ErrorCode),
						zap.String("ErrorMessage", *r.ErrorMessage),
					)
				} else {
					l = l.With(
						zap.String("ShardId", *r.ShardId),
						zap.String("SequenceNumber", *r.SequenceNumber),
					)
				}
				l.Info(fmt.Sprintf("Result[%d]", i))
			}
		}
		...
		p.Logger.With(
			zap.Int64("failures", failed),
			zap.String("backoff", duration.String()),
		).Warn("put failures")
